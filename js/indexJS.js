	                ...
	                if (array.length == 0) {
	                    ...
	                    for (var i = 0; i < response[subject].length; i++) {
	                        ...
	                        for (var j = 0; j < response[subject][i]["course"].length; j++) {
	                            stringForPrintHtml = stringForPrintHtml.concat('<div class="row"><div class="col-md-4 col-12 class-name" ><span style="box-sizing:content-box;">', response[subject][i]["course"][j]["topic"], '</span></div><div class="col-md-4 col-12 text-md-center room"><span>ห้อง ', response[subject][i]["course"][j]["room"], '</span></div><div class="col-md-4 col-12 button-time"><center><button type="button" class="btn btn-primary time-reseved" data-toggle="modal" data-target="#bookingModal" id="booking', response[subject][i]["course"][j]["course_id"], '" onclick="updateSeat(', response[subject][i]["course"][j]["course_id"], ')"><span>', response[subject][i]["course"][j]["time"], '</span></button></center></div></div><hr class="course-line">');
	                        }
