	                ...
	                if (array.length == 0) {
	                    ...
	                    for (showDate = 0; showDate < data.length; showDate++) {
	                        ...
	                        for (var i = 0; i < response[subject].length; i++) {
	                            ...
	                            for (var j = 0; j < response[subject][i]["course"].length; j++) {
	                                stringForPrintHtml = stringForPrintHtml.concat('<div class="row"><div class="col-md-4 col-6 class-name"><span>', response[subject][i]["course"][j]["topic"], '</span></div><div class="col-md-4 col-3 text-md-center room"><span>ห้อง ', response[subject][i]["course"][j]["room"], '</span></div><div class="col-md-4 col-3 button-time"><center><button type="button" class="btn btn-primary time-reseved" data-toggle="modal" data-target="#bookingModal" id="booking', response[subject][i]["course"][j]["course_id"], '" onclick="updateSeat(', response[subject][i]["course"][j]["course_id"], ')"><span>', response[subject][i]["course"][j]["time"], '</span></button></center></div></div><hr class="course-line">');
	                            }
	                            stringForPrintHtml = stringForPrintHtml + '</div><div class="card-footer"><p>ติดต่อ</p><p><img class="contract" src="./image/facebook.png" hspace = "10">' + response[subject][i]["facebook"] + '</p><p><img class="contract" src="./image/line.png" hspace = "10">' + response[subject][i]["line"] + '</p></div></div></div></div><br>';
	                            $("#courselist" + subject + response["day"]).append(stringForPrintHtml).fadeIn(1000);
	                        }
