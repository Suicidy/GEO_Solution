                ...
                success: function (data) {
                    ...
                    if (data.length) {
                        data.forEach(element => {
                            stringHtml = '';
                            stringHtml = stringHtml + '<div class="card" id="card'+element['course_id']+'"><div class="card-header" id="headingOne"><h5 class="mb-0"><button class="btn course-head col-12 collapsed" data-toggle="collapse" data-target="#collapse' + element['course_id'] + '" aria-expanded="false" aria-controls="collapseOne" id="coursehead' + element['course_id'] + '"><div class="row align-item-center"><div class="col-12 col-md-5 class-detail">' + element['topic'] + '</div><div class="col-12 col-md-3 class-detail">(' + element['date'] + '  ' + element['start_time'] + '-' + element['end_time'] + ' น.)</div><div class="col-12 col-md-2 class-detail">ห้อง ' + element['room'] + '</div><div class="col-1 col-md-2 info"><div class="btn btn-outline-info">Info</div></div><div class="col-12"><button type="button" class="btn btn-primary" data-toggle="modal" data-target="#requestModal" id="request' + element['course_id'] + '" onclick="getRequest('+element['course_id']+')">Show Request</button></div></div></button></h5></div></div></div>';
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.length) {
                ...
                data.forEach(element => {
                    stringHtml = '';
                    stringHtml = '<div class="row"><div class="col-md col-12 class-name"><span class="col-12">' + element['student_id'] + '</span></div><div class="col-md-3 col-12 class-name"><span class="col-12">'+element['title']+element['firstname']+' '+element['lastname']+'</span></div><div class="col-md col-12 class-name"><span class="col-12">เบอร์โทร '+element['tel']+'</span></div><div class="col-md col-12"><span class="col-12"><img src="/geo_solution/image/facebook.png" hspace = "10" class="contract"><span>'+element['facebook']+'</span></span></div><div class="col-md col-12 class-name "><span class="col-12"><img src="/geo_solution/image/line.png" hspace = "10" class="contract"><span>'+element['line']+'</span></span></div></div><hr class="course-line">';
